use anyhow::Result;
use crate::model::{NaryaConfig, ProxyGroup, GroupType};
...
impl SubscriptionParser {
    ...
    pub async fn parse(content: &str) -> Result<NaryaConfig> {
        ...
        if trimmed.starts_with("proxies:") || trimmed.contains("proxy-groups:") {
            ...
            if let Ok(decoded_bytes) = general_purpose::STANDARD.decode(trimmed.replace("\n", "").replace("\r", "")) {
    ...
    fn parse_clash(content: &str) -> Result<NaryaConfig> {
        ...
        if let Some(groups) = yaml.get("proxy-groups").and_then(|v| v.as_sequence()) {
            for g in groups {
                if let (Some(name), Some(g_type)) = (g.get("name").and_then(|v| v.as_str()), g.get("type").and_then(|v| v.as_str())) {
                    narya_config.groups.push(ProxyGroup {
                        ...
                        proxies: g.get("proxies")
                            ...
                            .map(|s| s.iter().filter_map(|p| p.as_str().map(|ps| ps.to_string())).collect())
                            .unwrap_or_default(),
                    });
                }
            }
    ...
    fn parse_plain_links(content: &str) -> Result<NaryaConfig> {
        ...
        for line in content.lines() {
            ...
            if line.starts_with("ss://") || line.starts_with("vmess://") || line.starts_with("trojan://") {
                // In a real implementation, we would parse these into nodes
                tracing::debug!("Found proxy link: {}", line.split("://").next().unwrap_or("unknown"));
            }
        }
        Ok(config)
    }
