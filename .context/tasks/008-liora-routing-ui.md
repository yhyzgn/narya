...
- 规则集新增持久化 `enabled` 生命周期；Liora `Switch` 可切换启停，禁用仍被规则引用的规则集会被拒绝，daemon 只缓存和编译启用且已验证的规则集。
- 规则集新增显式格式选择（sing-box binary、domain、ipcidr、classical）；mihomo 对启用的文本规则集生成 `rule-providers`，误用 sing-box binary 会明确拒绝。
- 分流规则编辑仍需覆盖更多 geosite/geoip/ACL 语义；当前阶段已完成规则集格式、启停、本地/HTTPS 已验证规则集缓存、AND 条件、分流组编辑和配置导入导出。
